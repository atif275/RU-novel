...
const AuthorProfile = ({ authorData, authorName }) => {
  ...
  const buttonStyles = theme === 'dark' ? 'bg-[#556b8b] text-white' : 'bg-[#337ab7] text-white';
  const textColor = theme === 'dark' ? 'text-[#c2a970]' : 'text-[#5e738b]';

  return (
    <div className={`p-[15px] font-[Open Sans, sans-serif] ${containerStyles}`}>
      <ToastContainer position="top-right" autoClose={5000} hideProgressBar={false} />
      <h2 className={`h-[48px] text-[18px] ${textColor} mb-[20px] font-semibold border-b ${borderStyles}`}>About the author</h2>
      ...
      <div className="flex flex-col gap-6 md:gap-0 md:flex-row md:justify-between">
        ...
        <div className="flex flex-col gap-4 justify-center items-center w-full md:w-1/6">
          ...
          <div className="flex flex-col justify-center items-center">
            <button className={`${buttonStyles} text-[12px] px-[10px] py-[5px] w-[100px] h-[30px] flex gap-1 items-center`}>
              <FaBook /> {authorData.fictions.length} Fictions
            </button>
            <button className={`${buttonStyles} text-[12px] px-[10px] py-[5px] w-[100px] h-[30px] mt-2 flex gap-1 items-center`}>
              <FaComment /> 0 Posts
            </button>
            <button className={`${buttonStyles} text-[12px] px-[10px] py-[5px] w-[100px] h-[30px] mt-2 flex gap-1 items-center`}>
              <FaListAlt /> 0 Threads
        ...
        <div className="flex flex-col justify-center w-full md:w-3/5">
          <div className="flex items-center gap-4 mb-2">
            <h3 className={`text-[36px] font-bold ${textColor}`}>{authorData.username}</h3>
            <button
              className={`${buttonStyles} text-[12px] px-[10px] py-[5px] w-[100px] h-[30px] mt-2`}
          ...
          <div className="flex flex-col md:flex-row mb-2 gap-1 md:gap-3">
            <p className="text-[14px] flex gap-1 items-center">
              <FaCalendar /> {new Date(authorData.createdAt).toLocaleString()}
            </p>
            <p className="text-[14px] flex gap-1 items-center">
              <FaLocationPin /> {authorData.location || 'Unknown'}
            </p>
          </div>
        ...
        <div className="flex flex-col justify-center w-full md:w-1/5">
          <h3 className={`text-[18px] text-[#e7505a] font-bold border-b ${borderStyles} pb-3`}>Achievements</h3>
