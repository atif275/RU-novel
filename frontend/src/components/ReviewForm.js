...
const ReviewForm = ({ bookName, onReviewAdded }) => {
  ...
  useEffect(() => {
    const fetchUserData = async () => {
      try {
        const response = await axios.get('http://localhost:5001/api/current-user', { withCredentials: true });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const ratingUpdateUrl = `http://localhost:5001/api/book/${encodeURIComponent(bookName)}/update-ratings`;
      const ratingParams = {
        overall: overallScore,
