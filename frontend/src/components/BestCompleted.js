// src/components/BestCompleted.js
    ...
    const fetchBooks = async () => {
      try {
        ...
        if (data.success && Array.isArray(data.data)) {
          ...
          console.error('Data is not an array or success flag is false');
        }
      } catch (err) {
      ...
      >
        <div className="w-full" style={{ margin: '0 0 10px' }}>
          <div className="flex items-center mb-4 h-12 border-b border-gray-300">
            <FaTrophy className="text-[#e26a6a] mr-2" size={20} />
            <h2
          ...
          {Array.isArray(books) ? books.slice(0, 10).map((book, index) => (
              ...
              >
                <div className="flex min-w-20 h-20 ml-[-20px] justify-center items-center">
                  <span className="text-[16px] text-[#e7505a] hover:underline" style={{fontFamily: 'Open Sans, sans-serif'}}>#{index + 1}</span>
                </div>
